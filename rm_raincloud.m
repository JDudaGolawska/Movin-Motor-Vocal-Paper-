...
% 8.02.2024 Joanna Duda-Golawska, some changes in ploting, added boxplots
% and labels
function h = rm_raincloud(data, colours1, colours2, labels, YTickLabel, plot_box_plot, plot_top_to_bottom, density_type, bandwidth)
%% check dimensions of data

[n_plots_per_series, n_series] = size(data);

% make sure we have correct number of colours
assert(all(size(colours1) == [n_series 3]), 'number of colors does not match number of plot series');

%% default arguments
if nargin < 6
    plot_box_plot  = 1;    % left-to-right plotting by default
end
...
if plot_box_plot
    spacing = 4 * mean(mean(cellfun(@max, ks)));%2 * mean(mean(cellfun(@max, ks)));
else
    spacing = 2.5 * mean(mean(cellfun(@max, ks)));%2 * mean(mean(cellfun(@max, ks)));
end
...
jit_width = spacing / 8;%8;


% TO-DO: This should probably not be hardcoded either...
% Although it can be overridden later
raindrop_size = 10;%100;

for i = 1:n_plots_per_series
    for j = 1:n_series
        jit{i,j} = j*jit_width + rand(1, length(data{i,j}))/size(data{i,j}, 2) * jit_width;
    end
end
...
for i = 1:n_plots_per_series
    for j = 1:n_series
        % plot patches
        %         h.p{i, j} = patch('Faces', faces{i, j}, 'Vertices', verts{i, j}, 'FaceVertexCData', colours(j, :),'HandleVisibility','off', 'FaceColor', 'flat', 'EdgeColor', 'none', 'FaceAlpha', 0.5);
        h.p{i, j} = patch('Faces', faces{i, j}, 'Vertices', verts{i, j}, 'FaceVertexCData', colours1(j, :),'HandleVisibility','off', 'FaceColor', 'flat', 'EdgeColor', 'none', 'FaceAlpha', 0.6);
        % scatter rainclouds
        %          h.b{i, j} = boxplot(data{i, j})
        ...
        if plot_box_plot
            h.s{i, j} = scatter(data{i, j}, -jit{i, j} + ks_offsets(i), 'MarkerFaceColor', colours2(j, :), 'MarkerEdgeColor', 'none', 'MarkerFaceAlpha', 0.8, 'SizeData', raindrop_size);
        end
    end
end
...
for i = 1:n_plots_per_series - 1 % We have n_plots_per_series-1 lines because lines connect pairs of points
    for j = 1:n_series
        h.l(i, j) = line(cell_means([i i+1], j), ks_offsets([i i+1]), 'LineWidth', 1, 'Color', colours1(j, :));
    end
end
...
for i = 1:n_plots_per_series
    for j = 1:n_series
        h.m(i, j) = scatter(cell_means(i, j), ks_offsets(i), 'MarkerFaceColor', colours1(j, :),'HandleVisibility','off', 'MarkerEdgeColor', [0 0 0], 'MarkerFaceAlpha', 1, 'SizeData', raindrop_size * 1.5, 'LineWidth', 1);
    end
end
...
if plot_box_plot
    ...
    for i = 1:n_plots_per_series
        for j = 1:n_series
            ...
            yl =  (min(-jit{i, j} + ks_offsets(i) )+max(-jit{i, j} + ks_offsets(i)))/2;
            ...
            box_pos = [Y(1) yl-(jit_width * 0.5) Y(2)-Y(1) jit_width];
            % median line
            h.b(i, j, 1)  = line([Y(3) Y(3)], [yl-(jit_width * 0.5) yl + (jit_width * 0.5)], 'col', 'k', 'LineWidth', line_width,'HandleVisibility','off');
            
            % whiskers
            h.b(i, j, 2)  = line([Y(2) Y(5)], [yl yl], 'col', 'k', 'LineWidth', line_width,'HandleVisibility','off');
            h.b(i, j, 4)  = line([Y(1) Y(4)], [yl yl], 'col', 'k', 'LineWidth', line_width,'HandleVisibility','off');
            
            % 'box' of 'boxplot'
            h.b(i, j, 3)  = rectangle('Position', box_pos);
            set( h.b(i, j, 3) , 'EdgeColor', 'k')
            set( h.b(i, j, 3) , 'LineWidth', line_width);
